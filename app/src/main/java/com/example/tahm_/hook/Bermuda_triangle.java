...
public class Bermuda_triangle extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mp.setLooping(true);
        mp.start();
        Intent go_b = getIntent();
        money = go_b.getIntExtra("money", 0);
        rod_level = go_b.getIntExtra("rod_level", 0);
        check_bait = go_b.getIntExtra("check_bait", 0);
        count_bait_normal = go_b.getIntExtra("count_bait_normal", count_bait_normal);
        count_bait_hidden = go_b.getIntExtra("count_bait_hidden", count_bait_hidden);
        plus_percent = go_b.getIntExtra("plus_percent", plus_percent);
        check();
    }

    Player_base player = new Player_base(money, rod_level, check_bait, count_bait_normal, count_bait_hidden, plus_percent);

    private int map_num=3;
